...
useEffect(() => { fetchProducts(selectedDate); }, [selectedDate]);
...
const handleAdd = async () => {
  ...
  try {
    await axios.post(API_URL, { name, initial_price, price, opening_stock, date: selectedDate });
...
const handleEntreeChange = async (id, value) => {
  await axios.put(`${API_URL}/entree/${id}`, { entree: Number(value), date: selectedDate });
  fetchProducts(selectedDate);
};

const handleSoldChange = async (id, value) => {
  await axios.put(`${API_URL}/sold/${id}`, { sold: Number(value), date: selectedDate });
  fetchProducts(selectedDate);
};
...
{[
  ...
  <div key={i} className="col-md-3">
    <div className="card border-0 shadow-lg" style={{ borderRadius: "16px", cursor: card.click ? "pointer" : "default" }} onClick={card.click}>
      <div className="card-body text-center">
        <p style={{ color: "#6B7280" }}>{card.title}</p>
        <h2 style={{ color: card.color, fontWeight: "700" }}>{formatNumber(card.value)}</h2>
      </div>
    </div>
  </div>
))}
...
<div className="card shadow-lg mb-4 border-0" style={{ borderRadius: "16px" }}>
  <div style={{ background: "#DC2626", color: "white", padding: "12px", borderRadius: "16px 16px 0 0" }}>Low Stock Drinks</div>
  <table className="table text-center mb-0">
    <thead style={{ background: "#1A2238", color: "white" }}>
      <tr><th>#</th><th>Drink</th><th>Remaining</th></tr>
    </thead>
    <tbody>
...
<div className="card shadow-lg mb-4 border-0" style={{ borderRadius: "16px" }}>
  <div className="card-body d-flex justify-content-between align-items-center">
    ...
    <div className="d-flex align-items-center gap-2">
      ...
      <button onClick={handleAdd} className="btn shadow ms-3" style={{ background: "#2563EB", color: "white", border: "none", padding: "8px 20px", borderRadius: "25px", fontWeight: "600", cursor: "pointer" }}>
        ➕ Add Drink
      </button>
    </div>
  </div>
...
<div className="card shadow-lg border-0" style={{ borderRadius: "16px" }}>
  <div className="table-responsive">
    <table className="table table-hover text-center align-middle">
      <thead style={{ background: "#1A2238", color: "white" }}>
        <tr><th>#</th><th>Drink</th><th>Cost</th><th>Price</th><th>Opening</th><th>Stock In</th><th>Total</th><th>Sold</th><th>Closing</th><th>Sales</th></tr>
      </thead>
      <tbody>
        {loading ? <tr><td colSpan="10">Loading...</td></tr>
        : products.length === 0 ? <tr><td colSpan="10">No drinks</td></tr>
        : products.map((p, i) => {
            const opening = Number(p.opening_stock || 0);
            const entree = Number(p.entree || 0);
            const sold = Number(p.sold || 0);
            ...
            return (
              <tr key={p.id} style={{ background: isLow ? "#FEE2E2" : "white" }}>
                ...
                <td>{opening}</td>
                <td><input type="number" className="form-control form-control-sm text-center" value={entree} onChange={(e) => handleEntreeChange(p.id, e.target.value)} /></td>
                <td style={{ fontWeight: "600" }}>{total}</td>
                <td><input type="number" className="form-control form-control-sm text-center" value={sold} onChange={(e) => handleSoldChange(p.id, e.target.value)} /></td>
